...
function App() {
  const finalRates = [];
  useEffect(() => {
    void axios("http://localhost:8000/rates").then((response) => {
      const rateList = ["EUR", "USD", "JPY", "BRL"];
      const rateDict = { rates: [] };

      rateList.forEach(function (item) {
        const tempData = [];
        rateDict.rates.push({ name: item });

        response.data.rates
          .filter(function (rate) {
            return rate.currency.short_name === item;
          })
          .forEach((it) => {
            // console.log(`{${item}: ${it.price}}`);
            // debugger;
            tempData.push([Date(it.date), it.price]);
          });
        const temp = rateDict.rates.filter((at) => {
          return at.name === item;
        })[0];
        finalRates.push({ ...temp, data: tempData });
      });
      ...
      Highcharts.chart("container", {
        ...
        title: {
          text: "Dashboard from Exchange rates",
        },
        xAxis: {
          type: "datetime",
        ...
        // TODO parei aqui, preciso fazer o gráfico levar em conta a finalRates com base de dados.
        // series: String(finalRates),
        series: [
          {
    ...
    <div className="max-w-[1344px] mx-auto flex flex-col items-center my-20">
      <figure className="highcharts-figure">
        ...
        <p className="highcharts-description">
          ...
          circles and triangles, but it is also possible to add your own custom
          symbols. In this chart, custom weather symbols are used on data points
          to highlight that certain temperatures are warm while others are cold.
        </p>
      </figure>
    </div>
  );
